...
const tests = [
  {
    ...
    task: {
      ...
      priority: 'normal',
      payload: {
        prompt: 'Evaluate the quality of this statement: "TiQology is building the future of legal tech."',
        model: 'chat-model',
      },
    },
  ...
  },
  {
    ...
    task: {
      ...
      priority: 'high',
      payload: {
        parentingPlan: 'Joint legal custody with Mother having primary physical custody. Father has weekend visitation every other weekend and Wednesday evenings.',
        communication: 'Parents communicate via email weekly to coordinate schedules. Tone is generally respectful with occasional delays in responses.',
        incidents: 'No documented safety incidents in the past 12 months. One missed pickup due to traffic, immediately communicated.',
        childProfile: '8-year-old boy, well-adjusted in school, enjoys sports. Strong relationship with both parents. No special needs.',
        model: 'chat-model',
      },
    },
    expectedStatus: 'completed',
  ...
  try {
    ...
    if (test.expectedError && result.error) {
      ...
      if (errorMatches) {
        ...
        log(`\n❌ FAILED - Expected error: ${test.expectedError}, got: ${result.error.code}`, 'red');
    ...
    if (passed) {
      ...
      if (result.result?.data) {
        ...
        if (data.taskTemplate) {
          log(`  Task Template: Generated (${data.taskTemplate.length} chars)`, 'yellow');
        }
        if (data.playbook) {
          log(`  Playbook: Generated (${data.playbook.length} chars)`, 'yellow');
        }
      }
      ...
      log(`\n❌ FAILED - Expected: ${test.expectedStatus}, got: ${actualStatus}`, 'red');
      return { name: test.name, passed: false, duration };
    }
  } catch (error) {
...
async function main() {
  ...
  results.forEach((result) => {
    ...
    log(`${icon} ${result.name}${durationStr}`, result.passed ? 'green' : 'red');
  });
