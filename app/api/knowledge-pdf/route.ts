...
/**
 * Special PDF handling endpoint - created as a temporary solution
 * to bypass potential routing issues with the main knowledge endpoint
 */
export async function POST(req: NextRequest) {
  console.log('PDF dedicated endpoint received a POST request');
  ...
  try {
    ...
    console.log(`Received form data with title: ${title}, has file: ${!!pdfFile}`);
    ...
    console.log(`Valid PDF file: ${pdfFile.name} (${pdfFile.size} bytes)`);
    ...
    return NextResponse.json(
      { error: 'Failed to create PDF document', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}