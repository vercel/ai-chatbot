...
A empresa busca democratizar o acesso a seguros através da tecnologia, conectando seguradoras a diversos canais de venda (corretores, bancos, e-commerces, fintechs e aplicativos) de forma simples e eficiente.`;
      ...
      // Retorna resposta com streaming real (efeito de digitação)
      const encoder = new TextEncoder();
      const stream = new ReadableStream({
        async start(controller) {
          // Divide a resposta em linhas para streaming gradual
    ...
    if (userContent.toLowerCase().includes('tendências') && userContent.toLowerCase().includes('insurtech')) {
      ...
      const insurtech2025Response = `📊 **Principais Tendências de Insurtech 2025 - Brasil e América Latina**
...
O mercado brasileiro de insurtech em 2025 está em expansão acelerada, com investimentos recordes e 206 startups ativas. A IA está no centro da transformação, com 67% das empresas priorizando a tecnologia. Os seguros embarcados crescem 10,4% ao ano, enquanto o Open Insurance democratiza o acesso a dados. O desafio principal é equilibrar inovação com compliance e segurança de dados.`;
      ...
      const stream = new ReadableStream({
        async start(controller) {
          // Divide a resposta em partes menores para streaming gradual
          const parts = [
            ...
            `- Process mining para otimizar fluxos\n\n`,
            `🎯 **Resumo Executivo**\n\n`,
            `O mercado brasileiro de insurtech em 2025 está em expansão acelerada, `,
            `com investimentos recordes e 206 startups ativas. `,
            `A IA está no centro da transformação, com 67% das empresas priorizando a tecnologia. `,
            `Os seguros embarcados crescem 10,4% ao ano, enquanto o Open Insurance democratiza o acesso a dados. `,
            `O desafio principal é equilibrar inovação com compliance e segurança de dados.`
          ];
          
          // Envia cada parte com delay para criar efeito de streaming
          for (const part of parts) {
            ...
            // Delay variável baseado no tamanho do conteúdo
            const delay = Math.min(30 + part.length * 2, 100); // Entre 30ms e 100ms
