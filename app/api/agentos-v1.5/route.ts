  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message:
              securityCheck.reason || "Request blocked by security check",
    ...
    if (loadMemory) {
      try {
        ...
        console.log(
          `[AgentOS] Loaded ${memoryContext.memories.length} memories for session`
        );
      } catch (error) {
    ...
    if (pipelineId) {
      ...
      return NextResponse.json({
        ...
        securityCheck:
          securityCheck.flags.length > 0 ? securityCheck : undefined,
