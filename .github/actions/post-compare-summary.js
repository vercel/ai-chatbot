...
// Uses the GITHUB_TOKEN provided by the runner.

import fs from "fs";
import https from "https";
import process from "process";

// Node 18+ has global fetch; fallback to a lightweight function if not available
  ...
  for (const r of report.results) {
    if (r.status === "REGRESSED") {
      annotations.push({
        ...
        message: `\n${r.query_name || r.query_file} regressed: baseline ${r.baseline_ms ?? "N/A"}ms → current ${r.current_ms ?? "N/A"}ms (${(r.pct_increase ?? 0).toFixed(1)}%)`,
      });
...
async function createCheckRun(
  ...
  if (!res.ok) {
    ...
    // Do not fail the workflow for annotation errors
    return;
  }
...
(async () => {
  try {
    ...
    process.exit(0); // Do not cause workflow failure
  }
})(); // .github/actions/post-compare-summary.js
