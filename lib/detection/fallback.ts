...
export async function mockRoofSegment(url: string, name: string): Promise<{
  maskUrl: string;
...
export async function mockPanelDetect(url: string, name: string): Promise<{
...
export async function mockDetectionAnalysis(files: Array<{ name: string; url: string }>): Promise<DetectionResult> {
  ...
  const totalRoof = items.reduce((sum, item) => sum + (item.metrics?.roof_coverage_m2 || 0), 0);
  const totalPanels = items.reduce((sum, item) => sum + (item.metrics?.panel_count || 0), 0);
  const avgConfidence = items.reduce((sum, item) => sum + (item.metrics?.confidence || 0), 0) / items.length;
