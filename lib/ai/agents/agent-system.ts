...
export class SolarCalculatorAgent {
		...
		try {
			...
			if (!capability) {
				return this.createResponse(
					"Não consegui identificar uma solicitação válida para cálculo solar.",
					0.3,
					startTime,
				);
			}
			...
			this.updateMemory(
				"conversation",
				{ input: userInput, response, toolResults },
				["solar", "calculation"],
			);
	...
	private analyzeIntent(input: string): string[] {
		...
		if (lowerInput.includes("solar") || lowerInput.includes("painel"))
			intents.push("solar");
		if (
			...
			intents.push("cost");
		if (lowerInput.includes("poupança") || lowerInput.includes("economia"))
			intents.push("saving");
		if (lowerInput.includes("retorno") || lowerInput.includes("roi"))
			intents.push("roi");
		if (lowerInput.includes("telhado") || lowerInput.includes("roof"))
			intents.push("roof");
		if (lowerInput.includes("técnico") || lowerInput.includes("technical"))
	...
	private async executeTools(
		...
		for (const toolName of capability.tools) {
			try {
				const providerResult = await loadBalancingService.selectProvider({
					modelType: "chat",
				});
				const provider =
					providerResult.success && providerResult.data
						? providerResult.data
						: {
								provider: "ollama",
								model: "qwen3:30b",
				...
				const result = await toolSystem.executeToolCall(
					toolCall,
					toolCall.context,
				);
				results.push(result);
	...
	private extractToolParameters(
		...
		if (userInput.includes("São Paulo") || userInput.includes("SP")) {
			...
			userInput.includes("Rio de Janeiro") ||
			userInput.includes("RJ")
		) {
		...
		if (billMatch) {
			params.monthlyBill = Number.parseFloat(
				`${billMatch[1]}.${billMatch[2] || "00"}`,
			);
		} else {
		...
		if (areaMatch) {
			params.roofArea = Number.parseFloat(
				`${areaMatch[1]}.${areaMatch[2] || "00"}`,
			);
		} else {
			...
			if (result.success && result.data) {
				...
				if (result.data.paybackYears < 5) {
					...
					combined.warnings.push(
						"Payback period longo - considere otimizar o sistema",
					);
				}
			}
		}
	...
	private formatSolarResponse(combinedData: any): any {
		...
		if (!calc) {
			return {
				type: "text",
				content:
					"Não foi possível calcular os dados solares. Por favor, forneça mais informações sobre sua conta de energia e área disponível.",
			};
		}
...
export class LeadQualificationAgent {
	...
	async processRequest(context: AgentExecutionContext): Promise<AgentResponse> {
		...
		try {
			...
			if (!capability) {
				return this.createResponse(
					"Não identifiquei uma solicitação relacionada à qualificação de leads.",
					0.3,
					startTime,
				);
			}
			...
			const errorMessage =
				error instanceof Error ? error.message : "Unknown error";
			this.updateLeadMemory("error", { error: errorMessage, context }, [
				"error",
				"lead",
			]);
			return this.createResponse(
				"Erro no processamento da qualificação de lead.",
				0.1,
				startTime,
			);
		}
	...
	private analyzeIntent(input: string): string[] {
		...
		if (lowerInput.includes("lead") || lowerInput.includes("prospect"))
			intents.push("lead");
		if (lowerInput.includes("qualif") || lowerInput.includes("score"))
			intents.push("qualification");
		if (lowerInput.includes("email") || lowerInput.includes("contact"))
			intents.push("communication");
	...
	private async generateLeadResponse(
		toolResults: any[],
		context: AgentExecutionContext,
	): Promise<AgentResponse> {
		const startTime = Date.now();
	...
	private formatLeadResponse(combinedData: any): any {
		...
		if (!analysis) {
			return {
				type: "text",
				content:
					"Não foi possível analisar os dados do lead. Verifique se todas as informações necessárias foram fornecidas.",
			};
		}
...
export class AgentOrchestrator {
	...
	private initializeAgents(): void {
		...
		this.agents.set("recommendation-agent", new RecommendationAgent());
	}
			...
			condition: (context) => {
				const input =
					context.conversationHistory[context.conversationHistory.length - 1]
						?.content || "";
				return (
					input.toLowerCase().includes("solar") ||
					input.toLowerCase().includes("painel")
				);
			},
			actions: [
				{
	...
	async processRequest(context: AgentExecutionContext): Promise<AgentResponse> {
		...
		return {
			...
			response: {
				type: "text",
				content:
					"Como posso ajudar com sua solicitação relacionada à energia solar?",
			},
			confidence: 0.5,
			executionTime: 0,
		};
	...
	private selectAgent(
		...
		if (
			context.currentPhase === "detection" ||
			lower.includes("detecção") ||
		...
		if (
			context.currentPhase === "investigation" ||
