-- 001_tiqology_optimizations.sql (CORRECTED FOR ACTUAL SCHEMA)
-- Create requested indexes and enable RLS + policies for TiQology
-- Non-destructive: uses IF NOT EXISTS where supported
-- Idempotent: safe to re-run
-- CORRECTED: Matches actual Drizzle schema column names

BEGIN;

-- =========================
-- INDEXES (19 -> 17 adjusted for actual schema)
-- =========================
...
-- Document table (id, createdAt, title, content, kind/text, userId)
-- Note: kind column is actually named "text" in schema (varchar "text")
...
-- Suggestion table (id, documentId, documentCreatedAt, originalText, suggestedText, description, isResolved, userId, createdAt)
...
-- =========================
-- RLS POLICIES (15)
-- Uses $policy$ inner delimiter to avoid conflicts with DO block $$
-- =========================
