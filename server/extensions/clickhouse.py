...
class ClickHouse:
    ...
    def _perform_initial_queries(self, app):
        ...
        rosters_query = "SELECT player_name, position, player_id FROM core.roster"
        res = client.query(rosters_query)
        app.config['ROSTER'] = pd.DataFrame(res.result_rows, columns=['player_name', 'position', 'player_id']).drop_duplicates(subset='player_id', keep='first')
...
def query_clickhouse(
    ...
    res = client.query_df(sql_query).to_json()
