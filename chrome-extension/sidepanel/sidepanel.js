...
function setupEventListeners() {
  ...
  // Listen for tab changes to update current tab info
...
async function updateCurrentTabInfo() {
  try {
    ...
    if (tabs.length > 0) {
      ...
      if (tabTitle) tabTitle.textContent = currentTab.title || 'Unnamed Tab';
      if (tabUrl) tabUrl.textContent = currentTab.url || '';
    }
  } catch (error) {
