...
export function Stock({ props: { symbol, price, delta } }: { props: Stock }) {
  ...
  return (
    <div
      className={`rounded-xl border bg-zinc-950 p-4 ${delta > 0 ? 'text-green-400' : 'text-red-600'}`}
    >
        ...
        >
          <defs>
            ...
            >
              <stop
                offset="0%"
                stopColor={`${delta > 0 ? '#34a853' : '#dc2626'}`}
                stopOpacity="0.38"
              ></stop>
              <stop
                offset="13%"
                stopColor={`${delta > 0 ? '#e6f4ea' : '#FFE6E6'}`}
                stopOpacity="0"
              ></stop>
            </linearGradient>
