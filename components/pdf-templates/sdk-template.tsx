...
type PDFContent = z.infer<typeof PDFSchema>;
...
const SdkPDFTemplate = ({ content, sum, ref }: Props) => {
    ...
    >
      ...
      <div className="font-bold">
        <div className="text-center">
          ...
          <div className="flex justify-between mt-8">{"Общество с ограниченной ответственностью".split("").map((symbol, index) => (<span key={index}>{symbol}</span>))}</div>
        </div>
        <hr className="border-t-2 border-black mt-2"/>
        <div className="flex justify-between mt-3">
          <div>
            628400, г.Сургут, ул.Маяковского, 12а<br />
            Магазин «Техносфера»<br />
            Тел.: (3462) 22-01-22, 23-80-08
          </div>
          <div className="text-right">
            ИНН 8602109264, р/с 40702810767170101413<br />
            Западно-Сибирский банк ОАО «Сбербанк России» г.Тюмень<br />
            к/с 30101810800000000651 • БИК 047102651<br />
            ОГРН 102860579226
          </div>
        </div>
      </div>
      ...
      <h1 className="text-center text-base mt-24 font-bold">Коммерческое предложение.</h1>
      ...
      <div className="mt-3">
        Всего наименований, на сумму {formatNumber(sum)} (Сорок шесть тысяч шестьсот шестьдесят рублей 00
        копеек), в том числе НДС {formatNumber(nds)}
      </div>
      ...
      <div>
        Срок действия предложения: до 31.12.2025. 2. Срок поставки товара: в срок до 31.10.2025 (включительно).
          ...
          размещением в местах хранения Заказчика, стоимость упаковки (тары), маркировки, страхование,
          таможенные платежи (пошлины), НДС, другие установленные налоги, сборы и иные расходы, связанные с
