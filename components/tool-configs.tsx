...
export const gmailToolConfig: ToolConfig = {
  ...
  getToolType: (toolCallId: string) => {
    if (toolCallId.includes('listGmailMessages'))
      return 'tool-listGmailMessages';
    if (toolCallId.includes('getGmailMessageDetails'))
      return 'tool-getGmailMessageDetails';
    return 'unknown';
  },
    ...
    switch (toolType) {
      case 'tool-listGmailMessages': {
        ...
        if (input.labelIds?.length)
          params.push(`labels: ${input.labelIds.join(', ')}`);
  ...
  getAction: (toolType: string, state: 'input' | 'output') => {
    ...
    switch (toolType) {
      ...
      case 'tool-getGmailMessageDetails':
        return isInput
          ? 'Fetching Gmail message details'
          : 'Fetched Gmail message details';
      default:
...
export const slackToolConfig: ToolConfig = {
  ...
  getToolType: (toolCallId: string) => {
    if (toolCallId.includes('listAccessibleSlackChannels'))
      return 'tool-listAccessibleSlackChannels';
    if (toolCallId.includes('fetchSlackChannelHistory'))
      return 'tool-fetchSlackChannelHistory';
    if (toolCallId.includes('getBulkSlackHistory'))
      return 'tool-getBulkSlackHistory';
    if (toolCallId.includes('getSlackThreadReplies'))
      return 'tool-getSlackThreadReplies';
    return 'unknown';
  },
...
export const transcriptToolConfig: ToolConfig = {
  ...
  getToolType: (toolCallId: string) => {
    if (toolCallId.includes('searchTranscriptsByKeyword'))
      return 'tool-searchTranscriptsByKeyword';
    if (toolCallId.includes('searchTranscriptsByUser'))
      return 'tool-searchTranscriptsByUser';
    if (toolCallId.includes('getTranscriptDetails'))
      return 'tool-getTranscriptDetails';
    return 'unknown';
  },
...
export const calendarToolConfig: ToolConfig = {
  ...
  formatParameters: (input: any) => {
    ...
    if (input.timeMin)
      params.push(`from: ${new Date(input.timeMin).toLocaleDateString()}`);
    if (input.timeMax)
      params.push(`to: ${new Date(input.timeMax).toLocaleDateString()}`);
    if (input.maxResults) params.push(`limit: ${input.maxResults}`);
...
export const mem0ToolConfig: ToolConfig = {
  ...
  getToolType: (toolCallId: string) => {
    ...
    if (toolCallId.includes('createMem0Project'))
      return 'tool-createMem0Project';
    ...
    switch (toolType) {
      ...
      case 'tool-createMem0Project':
        ...
        if (input.description)
          params.push(`description: "${input.description}"`);
        break;
      case 'tool-createMem0Memory':
        if (input.projectId) params.push(`project: ${input.projectId}`);
        if (input.messages) params.push(`messages: ${input.messages.length}`);
        if (input.metadata)
          params.push(`metadata: ${Object.keys(input.metadata).length} fields`);
        break;
    }
  ...
  getResultSummary: (output: any, input: any, toolType: string) => {
    ...
    try {
      if (typeof output.result === 'string') {
        ...
        const boundaryMatch = output.result.match(
          /<[^>]+>\s*({.*?})\s*<\/[^>]+>/s,
        );
        if (boundaryMatch) {
