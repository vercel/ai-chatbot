...
export function SolarCalculator({
  ...
  const [monthlyConsumption, setMonthlyConsumption] = React.useState<number>(300);
  ...
  const [results, setResults] = React.useState<SolarCalculationResults | null>(null);
  ...
  const calculateResults = () => {
    ...
    setTimeout(() => {
      ...
      const sunExposureFactor = 
        sunExposure === 'high' ? 1.2 : 
        sunExposure === 'medium' ? 1.0 : 0.8;
      
      // Cálculos básicos (simplificados para demonstração)
      const estimatedPowerNeeded = (monthlyConsumption / 120) * sunExposureFactor; // em kWp
      ...
      const adjustedPower = canFit ? estimatedPowerNeeded : (roofArea / 2) * 0.4;
  ...
  return (
    <Card className={cn("overflow-hidden", className)}>
      <CardHeader>
        <CardTitle className="text-2xl font-bold">Calculadora Solar YSH</CardTitle>
        <CardDescription>
          Descubra o potencial de economia com energia solar para sua residência ou negócio
        </CardDescription>
      </CardHeader>
        ...
        {results && (
          <div className="mt-6 p-4 bg-muted/50 rounded-lg space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2 border-t">
              <div className="space-y-1">
                <p className="text-sm text-muted-foreground">Sistema Recomendado</p>
                <p className="font-medium">{results.recommendedSystem.power.toFixed(2)} kWp</p>
              ...
              <div className="space-y-1">
                ...
                <p className="font-medium">{results.paybackPeriod.toFixed(1)} anos</p>
                <p className="text-sm text-muted-foreground">
                  Investimento: {formatCurrency(results.recommendedSystem.estimatedCost)}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t">
              ...
              <p className="text-sm">Redução de {Math.round(results.co2Reduction)} kg de CO₂ por ano</p>
            </div>
          </div>
        )}
      </CardContent>
